...
| 6 | Сборка APK (GitHub Actions) | **100 %** | APK собран и подписан: `apk/MDGram-9.3.3.apk` |
| 7 | Проверка APK в эмуляторе | **70 %** | джоба `emulator` (API 30): установка, запуск, краши, скриншот |
| 8 | Обновляемость (мерж апстрима, релизы) | **0 %** | стратегия обновлений |
...
## Что уже есть
- **APK собран**: `apk/MDGram-9.3.3.apk` (~78 МБ), подписан (v1+v2), 5 dex,
  ABI: arm64-v8a / armeabi-v7a / x86 / x86_64 — ставится и на телефон, и на эмулятор.
- Сборка полностью автоматическая: push с `.build-trigger` → APK в ветку + релиз.

## Дальнейшие шаги
1. Довести проверку в эмуляторе до скриншота (джоба `emulator`).
