...
public class LesserEqualFilter extends ComparisonFilter {
    public LesserEqualFilter(String str, Object obj) {
        super(str, obj);
    }
        ...
        for (Map.Entry<NitriteId, Document> entry : nitriteMap.entrySet()) {
            ...
            if (fieldValue != null) {
                ...
                if (fieldValue instanceof Comparable) {
                    ...
                    throw new FilterException(ErrorMessage.errorMessage(fieldValue + " is not comparable", ErrorCodes.FE_LTE_FIELD_NOT_COMPARABLE));
                }
            }
        }
    ...
    public Set<NitriteId> apply(NitriteMap<NitriteId, Document> nitriteMap) {
        if (this.field.equals(Constants.DOC_ID)) {
            ...
            if (nitriteId != null) {
                NitriteId floorKey = nitriteMap.floorKey(nitriteId);
                while (floorKey != null) {
                    linkedHashSet.add(floorKey);
                    floorKey = nitriteMap.lowerKey(floorKey);
                }
            }
            return linkedHashSet;
        } else if (this.nitriteService.hasIndex(this.field) && !this.nitriteService.isIndexing(this.field)) {
            return this.nitriteService.findLesserEqualWithIndex(this.field, this.comparable);
        } else {
