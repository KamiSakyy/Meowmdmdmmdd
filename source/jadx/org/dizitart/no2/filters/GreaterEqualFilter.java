...
public class GreaterEqualFilter extends ComparisonFilter {
    public GreaterEqualFilter(String str, Object obj) {
        super(str, obj);
    }
        ...
        for (Map.Entry<NitriteId, Document> entry : nitriteMap.entrySet()) {
            ...
            if (fieldValue != null) {
                ...
                if (fieldValue instanceof Comparable) {
                    ...
                    throw new FilterException(ErrorMessage.errorMessage(fieldValue + " is not comparable", ErrorCodes.FE_GTE_FIELD_NOT_COMPARABLE));
                }
            }
        }
    ...
    public Set<NitriteId> apply(NitriteMap<NitriteId, Document> nitriteMap) {
        if (this.field.equals(Constants.DOC_ID)) {
            ...
            if (nitriteId != null) {
                NitriteId ceilingKey = nitriteMap.ceilingKey(nitriteId);
                while (ceilingKey != null) {
                    linkedHashSet.add(ceilingKey);
                    ceilingKey = nitriteMap.higherKey(ceilingKey);
                }
            }
            return linkedHashSet;
