...
public final class RandomStringGenerator {
    ...
    public static class Builder implements org.apache.commons.text.Builder<RandomStringGenerator> {
        ...
        public Builder withinRange(int i, int i2) {
            zma.c(i <= i2, "Minimum code point %d is larger than maximum code point %d", Integer.valueOf(i), Integer.valueOf(i2));
            zma.b(i >= 0, "Minimum code point %d is negative", i);
            zma.b(i2 <= 1114111, "Value %d is larger than Character.MAX_CODE_POINT.", i2);
        ...
        public Builder withinRange(char[]... cArr) {
            this.characterList = new ArrayList();
            for (char[] cArr2 : cArr) {
                zma.c(cArr2.length == 2, "Each pair must contain minimum and maximum code point", new Object[0]);
                char c = cArr2[0];
                char c2 = cArr2[1];
                zma.c(c <= c2, "Minimum code point %d is larger than maximum code point %d", Integer.valueOf(c), Integer.valueOf(c2));
                for (int i = c; i <= c2; i++) {
    ...
    */
    ...
    public String generate(int i, int i2) {
        zma.b(i >= 0, "Minimum length %d is smaller than zero.", i);
        zma.c(i <= i2, "Maximum length %d is smaller than minimum length %d.", Integer.valueOf(i2), Integer.valueOf(i));
