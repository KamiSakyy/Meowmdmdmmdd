package org.telegram.tgnet;
/* loaded from: classes2.dex */
public class TLRPC$TL_botCommand extends a {
    public static int a = -1032140601;

    /* renamed from: a  reason: collision with other field name */
    public String f13945a;
    public String b;

    public static TLRPC$TL_botCommand f(b1 b1Var, int i, boolean z) {
        if (a != i) {
            if (!z) {
                return null;
            }
            throw new RuntimeException(String.format("can't parse magic %x in TL_botCommand", Integer.valueOf(i)));
        }
        TLRPC$TL_botCommand tLRPC$TL_botCommand = new TLRPC$TL_botCommand();
        tLRPC$TL_botCommand.d(b1Var, z);
        return tLRPC$TL_botCommand;
    }

    @Override // org.telegram.tgnet.a
    public void d(b1 b1Var, boolean z) {
        this.f13945a = b1Var.readString(z);
        this.b = b1Var.readString(z);
    }

    @Override // org.telegram.tgnet.a
    public void e(b1 b1Var) {
        b1Var.writeInt32(a);
        b1Var.writeString(this.f13945a);
        b1Var.writeString(this.b);
    }
}
