...
public final class ActivityUtils {
    ...
    private static boolean isIntentAvailable(Intent intent) {
        if (Utils.getApp().getPackageManager().queryIntentActivities(intent, Constants.CACHE_SIZE_DEFAULT).size() > 0) {
            return true;
        }
        return false;
    }
    ...
    private static Bundle getOptionsBundle(Fragment fragment, View[] viewArr) {
        ...
        return getOptionsBundle(i, viewArr);
    }
    ...
    private static Bundle getOptionsBundle(Activity activity, View[] viewArr) {
        ...
        if (viewArr != null && (length = viewArr.length) > 0) {
            sv6[] sv6VarArr = new sv6[length];
            for (int i = 0; i < length; i++) {
                View view = viewArr[i];
                sv6VarArr[i] = sv6.a(view, view.getTransitionName());
            }
            return x4.b(activity, sv6VarArr).c();
        }
        return null;
    }
