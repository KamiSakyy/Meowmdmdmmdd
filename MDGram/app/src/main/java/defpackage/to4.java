...
public class to4 {
    ...
    public to4(int i) {
        ...
        for (int i3 = 0; i3 <= this.N; i3++) {
            b(this.radius, i3);
            b(this.radiusNext, i3);
            this.progress[i3] = 0.0f;
        }
    }

    public void a(float f, float f2, float f3, float f4, Canvas canvas, Paint paint, float f5, float f6) {
        this.path.reset();
    ...
    public void c(float f, float f2) {
        for (int i = 0; i <= this.N; i++) {
            ...
            float f5 = f3 + (z00.MIN_SPEED * f4) + (f4 * f * z00.MAX_SPEED * f2);
            fArr[i] = f5;
            if (f5 >= 1.0f) {
                fArr[i] = 0.0f;
                float[] fArr2 = this.radius;
