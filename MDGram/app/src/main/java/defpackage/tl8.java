...
public class tl8 {
    ...
    private static final float[] SPLINE = new float[DataUtils.ERROR_TRANSACTION_LOCKED];
    ...
    static {
        ...
        sViscousFluidNormalize = 1.0f / o(1.0f);
    }
        ...
        if (currentAnimationTimeMillis < i) {
            ...
            if (i2 != 0) {
                if (i2 == 1) {
                    ...
                    float[] fArr = SPLINE;
                    float f3 = fArr[i3];
                    float f4 = f3 + (((f - f2) / ((i4 / 100.0f) - f2)) * (fArr[i4] - f3));
                    int i5 = this.mStartX;
                    int round = i5 + Math.round((this.mFinalX - i5) * f4);
                    this.mCurrX = round;
                    int min = Math.min(round, this.mMaxX);
                    this.mCurrX = min;
                    this.mCurrX = Math.max(min, this.mMinX);
                    int i6 = this.mStartY;
                    int round2 = i6 + Math.round(f4 * (this.mFinalY - i6));
                    this.mCurrY = round2;
    ...
    */
    public void d(int r17, int r18, int r19, int r20, int r21, int r22, int r23, int r24) {
        /*
            Method dump skipped, instructions count: 252
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: defpackage.tl8.d(int, int, int, int, int, int, int, int):void");
    }
    ...
    public float f() {
        return this.mVelocity - ((this.mDeceleration * n()) / 2000.0f);
    }
    ...
    public tl8(Context context, Interpolator interpolator) {
        this(context, interpolator, true);
    }

    public tl8(Context context, Interpolator interpolator, boolean z) {
        this.mFinished = true;
        this.mInterpolator = interpolator;
        this.mPpi = context.getResources().getDisplayMetrics().density * 160.0f;
        this.mDeceleration = b(ViewConfiguration.getScrollFriction());
        this.mFlywheel = z;
    }
}
