...
public class LesserThanFilter extends ComparisonFilter {
    public LesserThanFilter(String str, Object obj) {
        super(str, obj);
    }
        ...
        for (Map.Entry<NitriteId, Document> entry : nitriteMap.entrySet()) {
            ...
            if (fieldValue != null) {
                ...
                if (fieldValue instanceof Comparable) {
                    ...
                    throw new FilterException(ErrorMessage.errorMessage(fieldValue + " is not comparable", ErrorCodes.FE_LT_FIELD_NOT_COMPARABLE));
                }
            }
        }
    ...
    public Set<NitriteId> apply(NitriteMap<NitriteId, Document> nitriteMap) {
        if (this.field.equals(Constants.DOC_ID)) {
            ...
            if (nitriteId != null) {
                NitriteId lowerKey = nitriteMap.lowerKey(nitriteId);
                while (lowerKey != null) {
                    linkedHashSet.add(lowerKey);
                    lowerKey = nitriteMap.lowerKey(lowerKey);
                }
            }
            return linkedHashSet;
        } else if (this.nitriteService.hasIndex(this.field) && !this.nitriteService.isIndexing(this.field)) {
            return this.nitriteService.findLesserThanWithIndex(this.field, this.comparable);
        } else {
